...
export default function login() {
  ...
  const getUser = async (token: string) => {
    ...
    try {
      const resultFromCall = await fetch(
        process.env.EXPO_PUBLIC_USER_DOMAIN ?? " ",
        {
          method: "GET",
          headers: {
  ...
  const getTopArtists = async (token: string, sID: string) => {
    ...
    try {
      ...
      const jsonData = await resultFromCall.json();
      const artists = jsonData.items.map(
      ...
      if (error) {
        console.error(
          "ERROR - Could not update top10Artists and topGenres:",
          error.message
        );
      } else {
        console.log(
          "Successfully updated top10Artists and topGenres in user table!"
        );
      }
  ...
  const getRecentTracks = async (token: string, sID: string) => {
    ...
    try {
      ...
      const jsonData = await resultFromCall.json();
      const recentTracks = jsonData.items.map(
        (item: { track: any; name: any }) => item.track.name
      );
      ...
      if (error) {
        console.error(
          "ERROR - Could not update recentlyPlayed:",
          error.message
        );
      } else {
  ...
  return (
    <GestureHandlerRootView style={{ flex: 1 }}>
      <ImageBackground source={backgroundImage} style={styles.container}>
        ...
        />
        <Text style={{ height: 80, fontSize: 20 }}>
          Turn up the soundtrack of {"\n"}your life. Join the Beat Today
        </Text>

        <View style={styles.button}>
          <Button title="Start Now" onPress={() => handleSanpPress(0)} />
        </View>
        ...
        >
          <BottomSheetView style={styles.bottomSheetContainer}>
            ...
            <Text style={{ fontSize: 16, height: 35 }}>
              {" "}
              Login with Spofity and let the magic begin ...{" "}
            </Text>
